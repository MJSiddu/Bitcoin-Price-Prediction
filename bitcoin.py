...
def computeDelta(wt, X, Xi):
    ...
    for i in range(0,len(Xi)):
      q = Xi.iloc[i]
      sim_val = np.exp(similarity(X, q) * wt)
      summation_denom = summation_denom + sim_val
      summation_num = summation_num + sim_val * q[-1]
    res = summation_num/summation_denom
    return res
...
def similarity(p, q):
    ...
    p_values = p[0:-1]
    q_values = q[0:-1]
    mean_of_p = np.mean(p_values)
    mean_of_q = np.mean(q_values)

    for i in range(0,len(p_values)):
      sim = sim + ((p_values[i] - mean_of_p)*(q_values[i] - mean_of_q))

    denom = (len(p_values)*np.std(p_values)*np.std(q_values))
